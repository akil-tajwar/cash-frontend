...
export default function InterestRateReport() {
  ...
  const generateExcel = () => {
    exportToExcel(interestRateData, 'interest-rate-report-' + selectedDate)
  }

  const exportToExcel = (
    data: GetInterestRateReportType[],
    fileName: string
  ) => {
  ...
  const flattenData = (data: GetInterestRateReportType[]): any[] => {
                  ...
                  <TableBody>
                    {interestRateData.map((item, index) => (
                      <TableRow key={`interest-rate-${index}`}>
                        ...
                        <TableCell>BDT {formatIndianNumber(Number(item.totalLimit))}</TableCell>
                        <TableCell>
                    ...
                    <TableRow className="font-semibold bg-muted/20">
                      ...
                      <TableCell>-</TableCell>
                      <TableCell>
                        BDT {formatIndianNumber(Number(totals.totalLimit.toString()))}
                      </TableCell>
                      <TableCell>
                        BDT {formatIndianNumber(Number(totals.balanceOnDate.toString()))}
                      </TableCell>
                      <TableCell>
